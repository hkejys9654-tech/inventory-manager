(()=>{
  ...
  function outboundItemTotals(entries){
    const rows=new Map();
    entries.filter(e=>e.mode==='out').forEach(e=>{
      const key=`${e.cat}\u0000${e.name}`;
      if(!rows.has(key))rows.set(key,{cat:e.cat,name:e.name,outQty:0});
      rows.get(key).outQty+=Number(e.qty||0);
    });
  ...
  function monthlySummaryHtml(entries){
    const rows=outboundItemTotals(entries),body=rows.length?rows.map(r=>`<div class="itemsummary-row"><span class="sumname"><strong>${esc(r.name)}</strong><small>${esc(r.cat)}</small></span><span class="sumqty out">${r.outQty}</span></div>`).join(''):'<div class="itemsum-empty">이 달의 출고 기록이 없습니다.</div>';
    return `<div class="itemsummary"><div class="itemsummary-title">품목별 월간 출고 합계 · ${rows.length}개 품목</div><div class="itemsummary-row head"><span class="sumname">분류 · 품목</span><span>출고 합계</span></div>${body}</div>`;
  }
  function logCard(e){
  ...
  function buildLogImage(q){
    const showNotes=!exportSettings.hideNotes,RH=48,HH=54,TITLE=126,PAD=36,CW=showNotes?[165,150,365,90,90,260]:[165,150,625,90,90],TW=CW.reduce((a,b)=>a+b,0),isMonthly=q.period==='month',summary=isMonthly?outboundItemTotals(q.entries):[],SH=48,SRH=46,SECTION=42;
    const summaryHeight=isMonthly?SECTION+SH+SRH*Math.max(summary.length,1)+22:0,detailTitle=isMonthly?SECTION:0,H=TITLE+summaryHeight+detailTitle+HH+RH*q.entries.length+PAD+34,W=TW+PAD*2,S=H>7200?1:2;
    ...
    if(isMonthly){
      x.textAlign='left';x.fillStyle='#0f172a';x.font='700 20px "Noto Sans KR",sans-serif';x.fillText(`품목별 월간 출고 합계 · ${summary.length}개 품목`,PAD,y+SECTION/2);y+=SECTION;
      const SCW=[200,720,200],sheaders=['분류','품목','출고 합계'];left=PAD;x.textAlign='center';x.font='700 15px "Noto Sans KR",sans-serif';
      ...
      if(summary.length)summary.forEach((r,row)=>{left=PAD;const vals=[r.cat,r.name,String(r.outQty)];vals.forEach((v,i)=>{x.fillStyle=row%2?'#f8fafc':'#fff';x.fillRect(left,y,SCW[i],SRH);x.strokeStyle='#cbd5e1';x.lineWidth=1;x.strokeRect(left,y,SCW[i],SRH);x.fillStyle=i===2?'#dc3545':'#0f172a';x.font=i===2?'700 16px "JetBrains Mono",monospace':'14px "Noto Sans KR",sans-serif';x.fillText(fit(x,String(v),SCW[i]-12),left+SCW[i]/2,y+SRH/2);left+=SCW[i]});y+=SRH});
      else{x.fillStyle='#fff';x.fillRect(PAD,y,TW,SRH);x.strokeStyle='#cbd5e1';x.strokeRect(PAD,y,TW,SRH);x.fillStyle='#64748b';x.font='14px "Noto Sans KR",sans-serif';x.fillText('이 달의 출고 기록이 없습니다.',PAD+TW/2,y+SRH/2);y+=SRH}
  ...
  function exportReport(q){
    ...
    if(q.period==='month'){
      const itemRows=[['분류','품목','출고 합계']];outboundItemTotals(q.entries).forEach(r=>itemRows.push([r.cat,r.name,r.outQty]));
      const iws=XLSX.utils.aoa_to_sheet(itemRows);iws['!cols']=[{wch:15},{wch:34},{wch:12}];XLSX.utils.book_append_sheet(wb,iws,'품목별출고합계');
    }
